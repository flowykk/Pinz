...
struct AddMediaReviewView: View {
    @State private var viewModel: AddMediaReviewViewModel

    private let onBack: () -> Void
    private let onRetry: () async -> Void
    private let onApply: () async -> Void

    init(
        viewModel: AddMediaReviewViewModel,
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        if !viewModel.isLoading && viewModel.draftPins.isEmpty && !viewModel.hasFailed {
                            Text(PinzBaseStrings.AddMedia.Message.noDraftGroupsGeneratedYet)
                                .roundedFont(size: 14)
    ...
    private var gradientWithButtons: some View {
        Group {
            if viewModel.hasFailed && !viewModel.isLoading {
