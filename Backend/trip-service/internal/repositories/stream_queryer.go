...
// StreamQueryer treats NOGROUP as zero so a cold start with no consumers yet
// doesn't surface as an error in metric callbacks.
...
func (s StreamQueryer) XPending(ctx context.Context, stream, group string) (pinzotel.StreamPending, error) {
	...
	// XPending summary has no per-message idle; XPendingExt does.
