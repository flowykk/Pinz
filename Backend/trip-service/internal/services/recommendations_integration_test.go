...
func TestRecommendations_Integration(t *testing.T) {
	...
	insertPinForTest(t, sqlDB, topTripID, "sight", 55.7558, 37.6173, 5, "")
	insertPinForTest(t, sqlDB, topTripID, "sight", 55.7600, 37.6200, 3, "")
	insertPinForTest(t, sqlDB, topTripID, "nature", 55.7700, 37.6300, 2, "")
	insertPinForTest(t, sqlDB, topTripID, "nature", 55.7800, 37.6400, 1, "")
	insertPinForTest(t, sqlDB, topTripID, "entertainment", 55.7900, 37.6500, 1, "")
	insertPinForTest(t, sqlDB, topTripID, "entertainment", 55.8000, 37.6600, 1, "")
	// еда — 4 пина в разных кластерах.
	insertPinForTest(t, sqlDB, topTripID, "food", 55.8100, 37.6700, 4, "")
	insertPinForTest(t, sqlDB, topTripID, "food", 55.8200, 37.6800, 3, "")
	insertPinForTest(t, sqlDB, topTripID, "food", 55.8300, 37.6900, 2, "")
	insertPinForTest(t, sqlDB, topTripID, "food", 55.8400, 37.7000, 1, "")
	// дополнительно — Шопинг для проверки добора.
	insertPinForTest(t, sqlDB, topTripID, "shopping", 55.8500, 37.7100, 1, "")
	...
	insertPinForTest(t, sqlDB, secondTripID, "sight", 55.9000, 37.7200, 2, "")
	...
	insertPinForTest(t, sqlDB, oldTripID, "sight", 56.0000, 38.0000, 9, "should-not-appear")
	...
	insertPinForTest(t, sqlDB, parisTripID, "sight", 48.8566, 2.3522, 9, "")
...
// upsertCityForTest вставляет город и возвращает его id. geo_registry в
// trip-service — реплика, id назначается master'ом, поэтому в тесте генерим
// руками.
func upsertCityForTest(t *testing.T, db *sql.DB, name string, parentID *int) int {
	t.Helper()
	var id int
	require.NoError(t, db.QueryRow(`SELECT COALESCE(MAX(id), 0) + 1 FROM geo_registry`).Scan(&id))
	_, err := db.Exec(`INSERT INTO geo_registry (id, name, type, parent_id) VALUES ($1, $2, 'city', $3)`, id, name, parentID)
	require.NoError(t, err)
	return id
}
...
VALUES ($1, $2, 'vacation', 'summer', 'READY', 'public', $3, $4, $5, $6, true)
